...
class JackKnifeGraphNNDecoder(DependencyDecoder):
    ...
    def single_training_call(self, inputs, masks, truth, iters, is_train=True, is_tree=True):
        if type(inputs) == list:
            sent_len = len(inputs)
    ...
    def __call__(self, inputs, masks, truth, iters, is_train=True, is_tree=True):
        ...
        if type(inputs) is list:
            arc_mats, head_rels, dept_rels, arc_norms, gnn_losses_arr, rel_norms = [], [], [], [], [], []
            for x in inputs:
                arc_mat, head_rel, dept_rel, masks_flat, total_token, gnn_losses, sent_len, batch_size, arc_norm, rel_norm, flat_len = \
                    self.single_training_call(x, masks, truth, iters, is_train=is_train, is_tree=is_tree)
                
                arc_mats.append(arc_mat)
                head_rels.append(head_rel)
                dept_rels.append(dept_rel)
                arc_norms.append(arc_norm)
                rel_norms.append(rel_norm)
                gnn_losses_arr.append(gnn_losses)




            np_arc_mat = [x.as_array() for x in arc_mats]
            np_head_rels = [x.as_array() for x in head_rels]
            np_dept_rels = [x.as_array() for x in dept_rels]
            np_arc_norms = [x.as_array() for x in arc_norms]

            arc_mat = np.mean(np_arc_mat, axis=0)
            head_rel = np.mean(np_head_rels, axis=0)
            dept_rel = np.mean(np_dept_rels, axis=0)
            arc_norm = np.mean(np_arc_norms, axis=0)

            arc_mat = dy.inputTensor(arc_mat, batched=True)
            head_rel = dy.inputTensor(head_rel, batched=True)
            dept_rel = dy.inputTensor(dept_rel, batched=True)
            arc_norm = dy.inputTensor(arc_norm, batched=True)

        else:
            arc_mat, head_rel, dept_rel, masks_flat, total_token, gnn_losses, sent_len, batch_size, arc_norm, rel_norm, flat_len = \
                self.single_training_call(inputs, masks, truth, iters, is_train=is_train, is_tree=is_tree)
